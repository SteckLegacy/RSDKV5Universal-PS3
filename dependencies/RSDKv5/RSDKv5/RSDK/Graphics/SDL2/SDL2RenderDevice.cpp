...
{
    // Store the current logical size
    int w, h;
    SDL_RenderGetLogicalSize(renderer, &w, &h);

    // Set logical size to 0,0 to clear the whole window
    SDL_RenderSetLogicalSize(renderer, 0, 0);
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0xFF);

    // In triple-buffered scenarios, a single clear/present cycle isn't enough.
    // We do it three times to ensure all back-buffers are cleared and the front-buffer is clean.
    for (int i = 0; i < 3; ++i) {
...
void RenderDevice::CopyFrameBuffer()
{
    ...
    for (int32 s = 0; s < videoSettings.screenCount; ++s) {
        ...
        int32 width         = screens[s].size.x;
        int32 height        = screens[s].size.y;

        if (videoSettings.mirrorScreen) {
            for (int32 y = 0; y < height; ++y) {
                uint16 *srcRow = &frameBuffer[y * screens[s].pitch];
                uint16 *dstRow = &pixels[y * (pitch / sizeof(uint16))];
            ...
            for (int32 y = 0; y < height; ++y) {
                memcpy(&pixels[y * (pitch / sizeof(uint16))], &frameBuffer[y * screens[s].pitch], width * sizeof(uint16));
            }
        }
