...
bool32 RSDK::LoadFile(FileInfo *info, const char *filename, uint8 fileMode)
{
    ...
    if (modSettings.forceScripts && !info->externalFile) {
        std::string filePathStr(fullFilePath);
        const std::string scriptsPrefix = "Data/Scripts/";
        const std::string missionPrefix = "Data/Mission/";
        std::replace(filePathStr.begin(), filePathStr.end(), '\\', '/');

        if (ends_with(filePathStr, ".txt")) {
            std::string scriptName;
            if (filePathStr.rfind(scriptsPrefix, 0) == 0) {
                scriptName = filePathStr.substr(scriptsPrefix.length());
            } else if (filePathStr.rfind(missionPrefix, 0) == 0) {
                scriptName = filePathStr.substr(missionPrefix.length());
            }
            ...
            if (!scriptName.empty()) {
                ...
                if (it != scriptCache.end()) {
                    ...
                    info->fileBuffer = (uint8*)buffer.data();
                    info->fileSize = buffer.size();
                    info->readPos = 0;
                    info->usingFileBuffer = true;
                    info->file = (FileIO*)info->fileBuffer;
                    return true;
                }
            }
        }
