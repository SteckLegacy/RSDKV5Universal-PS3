    ...
    if (controller[CONT_ANY].keyStart.press || confirm) {
        switch (devMenu.selection) {
            case 0: {
                ...
                devMenu.mirrorScreen  = videoSettings.mirrorScreen;
...
{
    uint32 selectionColors[]           = { 0x808090, 0x808090, 0x808090, 0x808090, 0x808090, 0x808090, 0x808090 };
    selectionColors[devMenu.selection] = 0xF0F0F0;
    ...
    dy += 8;
    DrawDevString("Mirror Screen:", currentScreen->center.x - 96, dy, 0, selectionColors[4]);
    DrawDevString(devMenu.mirrorScreen ? "YES" : "NO", currentScreen->center.x + 80, dy, ALIGN_CENTER, 0xF0F080);

    dy += 16;
    DrawDevString("Confirm", currentScreen->center.x, dy, ALIGN_CENTER, selectionColors[5]);
    DrawDevString("Cancel", currentScreen->center.x, dy + 8, ALIGN_CENTER, selectionColors[6]);
    ...
    if (controller[CONT_ANY].keyDown.press) {
        if (++devMenu.selection > 6)
            devMenu.selection = 0;

        devMenu.timer = 1;
    }
    else if (controller[CONT_ANY].keyDown.down) {
        if (!devMenu.timer && ++devMenu.selection > 6)
            devMenu.selection = 0;
    ...
    switch (devMenu.selection) {
        ...
        case 5: // confirm
        {
            ...
            if (controller[CONT_ANY].keyStart.press || confirm) {
                ...
                videoSettings.mirrorScreen = devMenu.mirrorScreen;
                ...
                switch (devMenu.windowAspect) {
                    ...
                    case 0: width = 4 * videoSettings.pixHeight / 3; break; // 4:3
                    case 1: width = 3 * videoSettings.pixHeight / 2; break; // 3:2
                    case 2: width = 16 * videoSettings.pixHeight / 10; break; // 16:10
                    case 3: width = 5 * videoSettings.pixHeight / 3; break; // 5:3
                    case 4: width = 16 * videoSettings.pixHeight / 9; break; // 16:9
                }
                width &= 0x7FF8;
